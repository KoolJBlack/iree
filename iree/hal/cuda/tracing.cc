...
// will appear as a unique plot in the tracy UI with the given |stream_name|.
static void iree_hal_cuda_tracing_prepare_gpu_context(
  ...
  char* cloned_name = (char*)tracy::tracy_malloc(stream_name.size);
  memcpy(cloned_name, stream_name.data, stream_name.size);
  {
    auto* item = tracy::Profiler::QueueSerial();
    ...
    tracy::MemWrite(&item->gpuContextNameFat.size, stream_name.size);
    tracy::Profiler::QueueSerialFinish();
  }
  IREE_TRACE_ZONE_END(z0);
}
...
void iree_hal_cuda_tracing_sync(
  ...
  // Process remainder if we roll over the end of the ringbuffer.
  if (pool_rollover) {
    pool_rollover = iree_hal_cuda_tracing_query_returned_activity_buffers(
      tracing_context, &returned_buffer_base, &returned_buffer_head);
